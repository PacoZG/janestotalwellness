...
const SigninForm = () => {
  ...
  const handleLogin = async (event) => {
    ...
    try {
      ...
      if (user.userType === 'client') {
        dispatch(setNotification({
          message: `Bienvenido ${user.username}, tu sesión se cerrará automáticamente en 30 minutos`,
          title: 'Success',
          show: true
        }))
        username.reset()
  ...
  const handleResetTimers = () => {
    ...
    logoutTimeout = setTimeout(() => {
      dispatch(userLogout())
      dispatch(setNotification({
        message: 'Tu sesión ha expirado, comienza una nueva sesión.',
    ...
    <div className="static min-h-screen flex justify-center bg-gray-300 pt-20 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        <div>
          ...
          <h2 className="mt-6 text-center text-3xl font-extrabold text-gray-700">Entra en cuenta de usuario</h2>
        </div>
        ...
        <div className="relative w-auto my-6 mx-auto max-w-sm">
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-gray-50 outline-none focus:outline-none">
            ...
            <div className="flex items-start justify-between p-1 pl-4 pb-2 border-b border-solid border-blueGray-200 rounded-t">
              <h3 className="text-2xl font-semibold text-gray-700">Advertencia de caducidad de la sesión</h3>
            </div>
